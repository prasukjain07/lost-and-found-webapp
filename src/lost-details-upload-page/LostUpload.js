...
const LostUpload = (props) => {
  ...
  const [similarItems, setSimilarItems] = useState([]); // New state for similar items
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      // Save lost item to the database
      ...
      const response = await axios.post('http://localhost:5000/api/findMatchingFoundItems', {
        category,
        subcategory,
        itemName,
        place,
      ...
      setSimilarItems(response.data); // Set similar items in state
    ...
    <>
      ...
      {similarItems.length > 0 && (
        <div className="similar-items">
          <h2 style={{color: `${props.theme === 'dark' ? '#f5f5f5' : '#333'}`}}>Similar Found Items</h2>
          <div className="items-list">
            {similarItems.map((item) => (
              <div key={item._id} className="item">
                <img src={`http://localhost:5000/foundItemImages/${item.itemImage}`} alt={item.itemName} />
